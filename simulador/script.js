...
async function fetchTopDonates() {
	try {
		...
		if (donations.length > 0) {
			...
			donations = donations.sort((a, b) => b.valor - a.valor).slice(0, 15);

			const text = donations
				.map(
					(d) =>
						`${d.nome}: ${new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(d.valor)}`
				)
				.join("  •  ");
...
function renderBots(data) {
	...
	if (!data.bots || data.bots.length === 0) {
		botContainer.innerHTML =
			'<p style="text-align: center; padding: 20px;">Nenhum bot encontrado</p>';
		return;
	}
	...
	normalInfoText.innerHTML =
		"As ravenas <b>normais</b>, que você sempre usou! Os chips são comprados e mantidos por mim através das doações.<br><b>Apenas eu, o criador,</b> tenho acesso ao fluxo de dados deste bots.";
	...
	if (botsComunitarios.length > 0) {
		...
		comInfoText.innerHTML =
			"Estas ravenas são iniciativas de membros que doam seus chips e celulares para rodar a ravena.<br><b>O dono deste chip terá acesso às mensagens e fluxo de dados deste bot, se você não concorda com isso, pode remover o bot livremente.</b>";
	...
	if (botsVips.length > 0) {
		...
		vipInfoText.innerHTML =
			"Estas são ravenas que hospedo em agradecimento aos primeiros donates que ajudaram a solidificar a ravena, não estão mais disponíveis - estão aqui apenas para que os membros acompanhem o status.<br>⚠️ Os bots <i>vips</i> não recebem convites e nem respondem mensagens no pv!<br><br>";
...
function renderBotCard(botContainer, data, bot) {
	...
	if (bot.semPV || bot.semConvites) {
		const txtDetalhes = [bot.semPV && "PV Desabilitado", bot.semConvites && "Não recebe convites"]
			.filter(Boolean)
			.join(", ");
...
function processAnalyticsData(data) {
	...
	if (
		(!yearlyDates || yearlyDates.length === 0) &&
		data.yearly.series &&
		data.yearly.series.length > 0
	) {
...
function renderCharts(data) {
	const commonOptions = {
		...
		exporting: {
			enabled: true,
			buttons: {
				contextButton: { menuItems: ["downloadPNG", "downloadJPEG", "downloadPDF", "downloadCSV"] }
			}
		},
		...
		yAxis: {
			title: { text: "Mensagens", style: { color: "#b7b7c5" } },
			labels: { style: { color: "#b7b7c5" } },
...
function renderDailyChart(data, commonOptions) {
	...
	Highcharts.chart(container, {
		...
		xAxis: {
			...
			title: { text: "Hora do Dia", style: { color: "#b7b7c5" } }
		},
		tooltip: {
			formatter() {
...
function renderWeeklyChart(data, commonOptions) {
	...
	Highcharts.chart(container, {
		...
		xAxis: {
			...
			title: { text: "Dia da Semana", style: { color: "#b7b7c5" } }
		},
		tooltip: {
			formatter() {
...
function renderMonthlyChart(data, commonOptions) {
	...
	Highcharts.chart(container, {
		...
		xAxis: {
			...commonOptions.xAxis,
			categories: data.days,
			title: { text: "Dia do Mês", style: { color: "#b7b7c5" } }
		},
		tooltip: {
			formatter() {
...
function renderYearlyChart(data, commonOptions) {
	...
	Highcharts.chart(container, {
		...commonOptions,
		chart: { ...commonOptions.chart, type: "areaspline", zoomType: "x" },
		xAxis: {
			...commonOptions.xAxis,
			...
			labels: {
				...
				rotation: -45,
				step: Math.ceil(data.dates.length / 30)
			},
			title: { text: "Data", style: { color: "#b7b7c5" } }
		},
		tooltip: {
			formatter() {
				return `<b>${this.x}</b><br/>${this.series.name}: <b>${this.y}</b> msgs`;
