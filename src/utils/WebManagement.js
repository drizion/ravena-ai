...
class WebManagement {
  ...
  async getToken(token) {
    try {
      const row = await this.database.dbGet(this.DB_NAME, 'SELECT json_data FROM tokens WHERE token = ?', [token]);
  ...
  async saveToken(tokenData) {
    try {
      await this.database.dbRun(this.DB_NAME, 'INSERT OR REPLACE INTO tokens (token, group_id, json_data) VALUES (?, ?, ?)', 
