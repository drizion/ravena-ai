...
async function llmTranslate(text, targetLanguage, llmService) {
  ...
  try {
    const cached = await database.dbGet(dbName, 'SELECT translated_text FROM cached_translations WHERE hash = ?', [hash]);
    if (cached) {
...
6. DO NOT translate commands (strings starting with '!', '.', '/', or '#'). Return them exactly as is.`
  ...
  if (!translatedText) {
    try {
      ...
      const langCode = match ? match[1].toLowerCase() : (targetLanguage.length === 2 ? targetLanguage : 'en');
