...
const SequenceGenerators = {
    ...
    alphabet: () => {
        ...
        const seq = [];
        for(let i=0; i<6; i++) {
            if (startIdx + (i*skip) < pool.length)
                seq.push(pool[startIdx + (i*skip)]);
        }
    ...
    emojis: () => {
        ...
        const seq = [selected, selected+selected, selected+selected+selected, selected+selected+selected+selected];
    ...
    emojiAlgebra: () => {
        const combinations = [
            { eq: "👨 + 👩 = ?", ans: ["bebe", "bebê", "filho", "criança", "baby", "👶"], exp: "Homem + Mulher = Bebê" },
            { eq: "🌧️ + ☀️ = ?", ans: ["arco-iris", "arco iris", "arco íris", "rainbow", "🌈"], exp: "Chuva + Sol = Arco-íris" },
            { eq: "⚡ + 🌲 = ?", ans: ["fogo", "incendio", "incêndio", "fire", "🔥"], exp: "Raio + Árvore = Fogo" },
            { eq: "💧 + ❄️ = ?", ans: ["gelo", "ice", "cubo de gelo", "🧊"], exp: "Água + Frio = Gelo" },
            { eq: "🥚 + ⏳ = ?", ans: ["pinto", "pintinho", "galinha", "passaro", "pássaro", "ave", "🐣", "🐥", "🐔"], exp: "Ovo + Tempo = Nascimento/Ave" },
            { eq: "🐝 + 🌼 = ?", ans: ["mel", "honey", "🍯"], exp: "Abelha + Flor = Mel" },
            { eq: "🍇 + ⏳ = ?", ans: ["vinho", "wine", "🍷"], exp: "Uva + Tempo/Processo = Vinho" },
            { eq: "🐄 + 🥛 = ?", ans: ["queijo", "cheese", "🧀"], exp: "Leite de vaca processado = Queijo" },
            { eq: "🔴 + 🔵 = ?", ans: ["roxo", "purple", "violeta", "🟣", "🟪"], exp: "Vermelho + Azul = Roxo" },
            { eq: "🔵 + 🟡 = ?", ans: ["verde", "green", "🟢", "🟩"], exp: "Azul + Amarelo = Verde" },
            { eq: "⚪ + ⚫ = ?", ans: ["cinza", "grey", "gray", "🩶"], exp: "Branco + Preto = Cinza" },
            { eq: "🔪 + 🍅 = ?", ans: ["salada", "salad", "🥗", "fatias"], exp: "Tomate cortado = Salada" },
            { eq: "🥶 + 🌧️ = ?", ans: ["neve", "snow", "❄️", "🌨️"], exp: "Frio + Chuva = Neve" },
            { eq: "🌎 + 🚀 = ?", ans: ["lua", "moon", "espaço", "space", "marte", "martian", "alien", "👽", "🌑", "🌕"], exp: "Terra + Foguete = Viagem Espacial (Lua/Marte)" },
            { eq: "🐛 + ⏳ = ?", ans: ["borboleta", "butterfly", "🦋"], exp: "Lagarta + Tempo = Borboleta" },
            { eq: "👑 + 🦁 = ?", ans: ["rei leao", "rei leão", "lion king", "simba"], exp: "Referência ao filme Rei Leão" },
            { eq: "🧱 + 🧱 + 🧱 = ?", ans: ["parede", "muro", "casa", "construção", "wall", "house", "🏠"], exp: "Tijolos formam parede/casa" }
        ];
