...
async function aiCommand(bot, message, args, group) {
	...
	if (media && media.data) {
		...
		if (media.mimetype.includes("image")) {
			...
			completionOptions.systemContext =
				...
				"Você se chama ravenabot e deve inter esta imagem enviada no WhatsApp";
	...
	try {
		logger.debug(
			"[aiCommand] Tentando obter resposta do LLM",
			JSON.stringify(completionOptions).substring(0, 150)
		);
		...
		for (const pathToRemove of tempPathsToRemove) {
			try {
				...
				logger.error(
					`[aiCommand] Erro ao limpar arquivo temporário ${pathToRemove}:`,
					cleanupError
				);
			}
		}
