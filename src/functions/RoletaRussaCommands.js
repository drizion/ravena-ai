...
async function getGroupData(groupId) {
  let group = await database.dbGet(dbName, 'SELECT * FROM roleta_groups WHERE group_id = ?', [groupId]);
  ...
  if (!group) {
    ...
    await database.dbRun(dbName, `
      ...
      VALUES (?, ?, ?, 0, 0)
    `, [groupId, 300, now]);
    group = { group_id: groupId, timeout_time: 300, last_player_id: null, last_updated: now, total_tries: 0, total_deaths: 0 };
  }
...
async function getPlayerData(groupId, userId, userName = null) {
  let player = await database.dbGet(dbName, 'SELECT * FROM roleta_players WHERE group_id = ? AND user_id = ?', [groupId, userId]);
  ...
  if (!player) {
    ...
    await database.dbRun(dbName, 'UPDATE roleta_players SET user_name = ? WHERE group_id = ? AND user_id = ?', [userName, groupId, userId]);
...
async function mostrarRanking(bot, message, args, group) {
  try {
    ...
    const rankingSorte = await database.dbAll(dbName, `
    ...
    const rankingMortes = await database.dbAll(dbName, `
    ...
    if (rankingSorte.length > 0) {
      rankingSorte.forEach((jogador, index) => {
        ...
        const jogandoAtualmente = (jogador.current_tries || 0) > 0 ? ` *(${jogador.current_tries} atual)*` : '';
...
async function resetarRoletaRussa(bot, message, args, group) {
  try {
    ...
    const countResult = await database.dbGet(dbName, 'SELECT COUNT(*) as c FROM roleta_players WHERE group_id = ?', [groupId]);
...
async function definirTempoRoleta(bot, message, args, group) {
  try {
    ...
    await database.dbRun(dbName, 'UPDATE roleta_groups SET timeout_time = ? WHERE group_id = ?', [segundos, groupId]);
